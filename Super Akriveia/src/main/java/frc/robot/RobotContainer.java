...
public class RobotContainer extends Controller {
  ...
  private void configureButtonBindings() {
    ...
    // Drivetrain buttons
    getDefenseButton().whenHeld(new SwerveDefense());
    getVisionButton().whenHeld(new SwerveVision( 
      () -> getForward(), 
      () -> getStrafe()));
    getResetGyroButton().whenHeld(new SwerveResetGyro());
    getResetGyroButton().whenPressed(new SwerveResetGyro());
    getLockSpinButton().whenPressed(new SwerveLockedSpin());
    getTurboButton().whenHeld(new SwerveTurboOn());
    getTurboButton().whenReleased(new SwerveTurboOff());
    getOrbitButton().whenHeld(new SwerveOrbit(
      () -> getOrbitSpeed(),
      () -> getPOVTransStick()));

    getIntakeSpinButton().whenActive(new IntakeRoll());
    getIntakeReverseButton().whenActive(new IntakeReverse());
  
    getDrumUpButton().whenActive(new DrumVelocitySpeed());
    getDrumDownButton().whenActive(new DrumVelocitySlow());
  
    getTransferStraightButton().whenActive(new BallTransferStraight());
    getTransferPivotButton().whenActive(new BallTransferPivotAndRoll());
  
    getIntakeHarvestButton().whenActive(new IntakeHarvest());
    getIntakeHarvestResetButton().whenActive(new IntakeHarvestReset());
  
    // getDrumAgitateButton().whenActive(new DrumAgitate());
    getDrumAgitatePreShootButton().whenActive(new DrumAgitatePreShoot());
  
    getDrumInputResetButton().whenActive(new DrumInputReset());
    getDrumCCWButton().whenActive(new DrumCCW());
  
    getPOVUp().whenHeld(new AngledShooterUp());
    getPOVDown().whenHeld(new AngledShooterDown());
    getPOVLeft().whenHeld(new FlywheelSlow());
    getPOVRight().whenHeld(new FlywheelSpeed());

    getAngledShooterOffButton().whenActive(new AngledShooterOff());
  
    getShootButton().whenActive(new Shoot());
    getResetShootButton().whenActive(new ShootReset());
